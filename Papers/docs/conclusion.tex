...
为了捕捉这种结构性差异, 未来的研究可参考双对数结构嵌套均值模型, 将弹性系数参数化为品类特征的函数\citep{hua2021markdownsecommercefreshretail}. 假设价格弹性 $\theta$ 并非由常数决定, 而是由品类指示向量 $\vb{L}_i$ 动态调节:
\begin{align} \label{eq:hte_model}
    \theta_i = \theta_{\text{base}} + \vb*{\delta}^\top \vb{L}_i
\end{align}

式 \eqref{eq:hte_model} 中, $\theta_{\text{base}}$ 为基准弹性, $\vb*{\delta}$ 为不同品类相对于基准的弹性偏移量 (Effect Modifiers). 在 DML 的第二阶段推断中, 我们可以将正交化后的残差回归方程扩展为包含交互项的形式:
\begin{align}
